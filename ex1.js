var myArray = []; // Declared array to store tasks
...
function listAllTasks() {
    for (var i = 0; i < myArray.length; i++) { // loops through until reaches the length of array
        console.log("Task " + (i + 1) + ": " + myArray[i]); // Prints each of the contents
    }
}
function deleteTask(task) {
    var index = myArray.indexOf(task); // Finds where a specific task is
